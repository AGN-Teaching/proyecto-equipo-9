...
- En esta práctica, se logró diseñar un modelo orientado a objetos sólido que aborda de manera efectiva la problemática particular de un sistema de taxis. Este enfoque proporciona una base sólida para el desarrollo futuro del sistema, asegurando una gestión ordenada, segura y eficiente de las reservaciones, del usuario, las unidades de taxis y la interacción general con el sistema La implementación de restricciones para el registro de usuarios y la gestión de promociones para clientes recurrentes refuerza la seguridad y fidelización de los usuarios en el sistema de reservaciones. 

### Gabriel

- El objetivo de este proyecto fue implementar el uso de clases y objetos para desarrollar un sistema de taxis cómo comúnmente lo vemos por lo que fue necesario desde utilizar la abstracción de conceptos en nuestro mundo y llevarlo a un lenguaje de programación, hasta formular los métodos que serían necesarios para tener un modelo que tuviese un correcto funcionamiento como en el caso de las reservaciones las cuales son el producto de la implementacion de metodos de clase partiendo desde las relaciones que existen entre cada una de ellas. Por otro lado también fue importante establecer las restricciones de nuestro problema para poder delimitar los aspectos en los que más necesitamos enfocarnos así como desarrollar un modelo orientado a objetos basado en ello; la realización de este proyecto requirió de aplicar todos los conocimientos adquiridos a lo largo de la UEA e incluso complementar con lo aprendido en UEAS pasadas,   el desarrollo de las clases requirió de idear principalmente las relaciones que tendrían entre ellas así como los diferentes métodos que necesitábamos implementar en cada una, fue importante considerar los tipos de relaciones existentes pues el menú por ejemplo tiene una relación de composición con la clase sistema debido a que es necesario inicializar una instancia de tipo Sistema para que la clase Menu funcione de manera correcta.

- Requerimos un mayor esfuerzo al momento de realizar esta práctica debido a las especificaciones del problema; los conceptos que fueron más útiles en este caso para la implementación fue la herencia y el polimorfismo en mayor medida pues por ejemplo para los diferentes tipos de unidades necesitabamos que un mismo metodo pudiese realizar diferentes procesos en base al contexto que lo necesitáramos. Podemos decir que el implementar este paradigma de programación facilita el manejo del programa al tener clases en diferentes scripts pues al solucionar un error por ejemplo es más fácil manejar un código compacto que tener que modificar todo de manera lineal.
